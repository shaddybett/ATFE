...
function TeacherDashboard() {
  const { deleteClass, loading, setLoading, API_URL, setClasses, setError, classes } = useContext(ClassContext) 
  ...
  return (
    <div className={(loading || showForm || showEditClassForm) ? 'overflow-hidden h-full max-h-[100vh]' :''}>
      ...
      <section onClick={()=>setHoveredCard(null)} className="w-full max-w-7xl mx-auto px-4 py-12 ">
      ...
      <section className="w-full max-w-7xl mx-auto px-4 py-4">
        ...
        <div className="flex flex-col gap-6 py-6">
          {classes?.map((classData, index) => (
              <div
                    key={classData.id}
                    className="class-card relative flex gap-6 items-end w-full max-w-xl bg-light-orange hover:bg-t-orange transition px-4 pt-10 pb-2 rounded-lg"
                  >
                    <Link className='mr-auto w-full' to={`/class/${classData.id}`}>
                    <div >
                    ...
                    <ListGroup className={`list-group ${hoveredCard === index ? 'opacity-100 scale-100' : 'scale-0'}`}>
